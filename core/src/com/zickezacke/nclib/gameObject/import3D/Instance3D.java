 ...
 * </p>
 *
 * The Instance3D creates a full copy of all materials, nodes and animations.
 * @author badlogic, xoppa */
public class Instance3D implements RenderableProvider {
    /** Whether, by default, {@link NodeKeyframe}'s are shared amongst {@link Model} and Instance3D. Can be overridden per
     * Instance3D using the constructor argument. */
    ...
    public Instance3D (final Model model) {
    ...
    public Instance3D (final Model model, final String nodeId, boolean mergeTransform) {
     ...
     * @param nodeId The ID of the root {@link Node} of the {@link Model} for the instance to contain
     * @param mergeTransform True to apply the source node transform to the instance transform, resetting the node transform. */
    public Instance3D (final Model model, final Matrix4 transform, final String nodeId, boolean mergeTransform) {
     ...
     * @param mergeTransform True to apply the source node transform to the instance transform, resetting the node transform. */
    public Instance3D (final Model model, final String nodeId, boolean parentTransform, boolean mergeTransform) {
     ...
     * @param parentTransform True to apply the parent's node transform to the instance (only applicable if recursive is true).
     * @param mergeTransform True to apply the source node transform to the instance transform, resetting the node transform. */
    public Instance3D (final Model model, final Matrix4 transform, final String nodeId, boolean parentTransform,
     ...
     * @param parentTransform True to apply the parent's node transform to the instance (only applicable if recursive is true).
     * @param mergeTransform True to apply the source node transform to the instance transform, resetting the node transform. */
    public Instance3D (final Model model, final String nodeId, boolean recursive, boolean parentTransform,
                          boolean mergeTransform) {
     ...
     * @param parentTransform True to apply the parent's node transform to the instance (only applicable if recursive is true).
     * @param mergeTransform True to apply the source node transform to the instance transform, resetting the node transform. */
    public Instance3D (final Model model, final Matrix4 transform, final String nodeId, boolean recursive,
    ...
    public Instance3D (final Model model, final String... rootNodeIds) {
        this(model, null, rootNodeIds);
    }

    /** Constructs a new ModelInstance with only the specified nodes and materials of the given model. */
    public Instance3D (final Model model, final Matrix4 transform, final String... rootNodeIds) {
        this.model = model;
        this.transform = transform == null ? new Matrix4() : transform;
        if (rootNodeIds == null)
    ...
    /** Constructs a new ModelInstance with only the specified nodes and materials of the given model. */
    public Instance3D (final Model model, final Array<String> rootNodeIds) {
        this(model, null, rootNodeIds);
    }

    /** Constructs a new ModelInstance with only the specified nodes and materials of the given model. */
    public Instance3D (final Model model, final Matrix4 transform, final Array<String> rootNodeIds) {
        this(model, transform, rootNodeIds, defaultShareKeyframes);
    }

    /** Constructs a new ModelInstance with only the specified nodes and materials of the given model. */
    public Instance3D (final Model model, final Matrix4 transform, final Array<String> rootNodeIds, boolean shareKeyframes) {
        this.model = model;
        this.transform = transform == null ? new Matrix4() : transform;
        copyNodes(model.nodes, rootNodeIds);
        copyAnimations(model.animations, shareKeyframes);
    ...
    public Instance3D (final Model model, Vector3 position) {
        this(model);
        this.transform.setToTranslation(position);
    }

    /** Constructs a new ModelInstance at the specified position. */
    public Instance3D (final Model model, float x, float y, float z) {
        this(model);
        this.transform.setToTranslation(x, y, z);
    }

    /** Constructs a new ModelInstance with the specified transform. */
    public Instance3D (final Model model, Matrix4 transform) {
        this(model, transform, (String[])null);
    }

    /** Constructs a new ModelInstance which is an copy of the specified ModelInstance. */
    public Instance3D (ModelInstance copyFrom) {
        this(copyFrom, copyFrom.transform.cpy());
    }

    /** Constructs a new ModelInstance which is an copy of the specified ModelInstance. */
    public Instance3D (ModelInstance copyFrom, final Matrix4 transform) {
        this(copyFrom, transform, defaultShareKeyframes);
    }

    /** Constructs a new ModelInstance which is an copy of the specified ModelInstance. */
    public Instance3D (ModelInstance copyFrom, final Matrix4 transform, boolean shareKeyframes) {
    ...
    /** Copy source animations to this Instance3D
    ...
    /** Copy the source animation to this Instance3D
    ...
    public Quaternion getRotation() {return this.transform.getRotation(new Quaternion());}

    public void setRotation(Vector3 axis, float degree) {
        this.transform.rotate(axis, degree);
    }

    public Vector3 getScale(){
        return this.transform.getScale(new Vector3());
    }
